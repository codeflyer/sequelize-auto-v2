  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(!self.options.typescript){
        ...
        text[table] += "module.exports = function(DataTypes) {\n";
        text[table] += spaces + "return {modelName: '" + tableName + "', attributes: {\n";
      } else {
      ...
      _.each(fields, function(field, i) {
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            ...
            if (!hasPrimaryKey && self.tables[table][field][attr] === true && (! _.has(self.tables[table][field], 'foreignKey') || (_.has(self.tables[table][field], 'foreignKey') && !! self.tables[table][field].foreignKey.isPrimaryKey))) {
              hasPrimaryKey = true;
            ...
            text[table] += spaces + spaces + spaces + attr + ": " +
              (self.tables[table][field][attr] !== null
                ? '"' + self.tables[table][field][attr].replace(/(?:\r\n|\r|\n)/g, '\\n').replace(/"/g, '\\"') + '"'
                : "null");
          } else {
      ...
      if(self.options.typescript) {
        typescriptFiles[1] = tsHelper.model.generateTableModels(_.keys(text), self.options.spaces, self.options.indentation, self.options.camelCase, self.options.camelCaseForFileName);
      }
